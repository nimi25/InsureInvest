...
.note{color:#64748b;margin-top:-.45rem;margin-bottom:1rem}
</style>
...
fig = go.Figure()
fig.add_trace(go.Candlestick(x=stock.Date, open=stock.Open, high=stock.High, low=stock.Low, close=stock.Close, name=selected))
...
explanation(f"<b>What this shows:</b> Candlesticks show daily price movement. The 50-day and 200-day SMAs describe short- and long-term trend, while Bollinger Bands show a volatility envelope. <b>{selected}:</b> the current price is <b>₹{price:,.2f}</b>, {('above' if price > float(latest['SMA_50']) else 'below')} its 50-day average and {('above' if price > float(latest['SMA_200']) else 'below')} its 200-day average.")
...
explanation(f"<b>What this shows:</b> RSI ranges from 0 to 100 and is commonly used to identify momentum extremes. <b>{selected}:</b> RSI is <b>{rsi:.1f}</b>, indicating <b>{rsi_state} momentum</b> at the latest observation.")
...
explanation(f"<b>What this shows:</b> MACD compares short- and long-term price momentum. <b>{selected}:</b> MACD is <b>{macd:.2f}</b> versus a signal value of <b>{signal:.2f}</b>, giving a <b>{macd_state} momentum reading</b>.")

trend = "positive" if price > float(latest["SMA_50"]) and price > float(latest["SMA_200"]) else "weak" if price < float(latest["SMA_50"]) and price < float(latest["SMA_200"]) else "mixed"
explanation(f"<b>Technical summary for {selected}:</b> Trend is <b>{trend}</b>, RSI is <b>{rsi_state}</b>, MACD momentum is <b>{macd_state}</b>, and the latest price is inside the Bollinger Bands. This is a technical snapshot, not a standalone buy/sell signal.")
...
explanation(f"<b>What this table shows:</b> Sharpe measures historical excess return per unit of volatility, Beta measures sensitivity to NIFTY 50 movements, and maximum drawdown captures the largest historical decline from a previous peak. <b>{selected}</b> has Sharpe <b>{metrics['Sharpe Ratio']:.2f}</b> and Beta <b>{metrics['Beta']:.2f}</b>.")
...
explanation(f"<b>What this shows:</b> Both series start at 100, making relative growth easy to compare. Over this historical period, <b>{selected}</b> has <b>{relative}</b> the NIFTY 50 based on the displayed indexed endpoints.")

st.header("3. Insurance Peer Comparison")
st.markdown('<div class="note">Compare the selected company with the pure-insurance companies in the dataset.</div>', unsafe_allow_html=True)
...
if peer_rows:
    ...
    if selected in peers["Company"].values:
        rank = int(peers.reset_index(drop=True).index[peers.reset_index(drop=True)["Company"] == selected][0] + 1)
        explanation(f"<b>What this shows:</b> The ranking compares historical risk-adjusted performance. <b>{selected}</b> ranks <b>#{rank} of {len(peers)}</b> by Sharpe ratio in the currently available pure-insurance peer set.")
    else:
        explanation(f"<b>What this shows:</b> {selected} is not currently available in the pure-insurance peer subset, so a peer rank is not assigned.")
else:
    st.warning("No peer data is currently available.")

st.header("4. Efficient Frontier")
st.markdown('<div class="note">Historical portfolio combinations showing the trade-off between annualized return and volatility.</div>', unsafe_allow_html=True)
...
if returns.shape[1] >= 3:
    ...
    efig = go.Figure()
    efig.add_trace(go.Scatter(x=port_vol, y=port_ret, mode="markers", marker=dict(size=4, color=sharpe, colorscale="Viridis", showscale=True, colorbar=dict(title="Sharpe")), name="Simulated portfolios"))
    ...
    explanation(f"<b>What this shows:</b> Each dot is a simulated portfolio of the pure-insurance companies. The horizontal axis is historical annualized volatility and the vertical axis is historical annualized return. The star is the simulated portfolio with the highest Sharpe ratio. <b>For {selected}:</b> the diamond shows its standalone historical risk/return position when it is in the peer set. This is a historical portfolio-efficiency exercise, not a forecast.")
else:
...
st.markdown('<div class="note">A probabilistic 1-year price-path simulation based on historical drift and volatility.</div>', unsafe_allow_html=True)
sims = st.slider("Number of simulations", 1000, 10000, 3000, 500, key="mc_sims")
days = 252
hist_ret = daily_returns(close)
...
explanation(f"<b>What this shows:</b> The simulation generates possible 1-year price paths using {sims:,} trials and the selected company's historical daily return distribution. The median simulated ending price is <b>₹{percentiles[2]:,.2f}</b>; the 5th–95th percentile range is <b>₹{percentiles[0]:,.2f}–₹{percentiles[4]:,.2f}</b>. These are scenarios, not predicted prices or guaranteed outcomes.")

st.header("6. Black-Scholes Option Valuation")
st.markdown('<div class="note">Theoretical European call and put values using the selected company's current price and historical volatility.</div>', unsafe_allow_html=True)
b1, b2, b3, b4 = st.columns(4)
with b1:
    strike = st.number_input("Strike price (₹)", min_value=0.01, value=round(price, 2), step=1.0)
with b2:
    expiry = st.number_input("Time to expiry (years)", min_value=0.01, value=1.0, step=0.25)
with b3:
    rate = st.number_input("Risk-free rate", min_value=0.0, max_value=1.0, value=0.06, step=0.01, format="%.2f")
with b4:
    dividend = st.number_input("Dividend yield", min_value=0.0, max_value=1.0, value=0.0, step=0.01, format="%.2f")
...
if vol > 0 and T > 0:
    ...
    explanation(f"<b>What this shows:</b> Black-Scholes estimates theoretical European option values from price, strike, time, interest rate, dividend yield and volatility. For {selected}, the model gives a theoretical <b>call of ₹{call:,.2f}</b> and <b>put of ₹{put:,.2f}</b>. The valuation is model-based and depends strongly on the assumptions supplied; it is not the observed market option price.")
else:
    st.warning("Black-Scholes requires positive historical volatility and a positive expiry.")
