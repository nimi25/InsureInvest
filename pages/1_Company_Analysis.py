...
risk = pd.DataFrame({
    ...
    selected: [metrics["Annual Return"], metrics["Annual Volatility"], metrics["Sharpe Ratio"], metrics["Maximum Drawdown"], metrics["Beta"]],
    "NIFTY 50": [bench_metrics["Annual Return"], bench_metrics["Annual Volatility"], bench_metrics["Sharpe Ratio"], bench_metrics["Maximum Drawdown"], 1.0],
})
...
if peer_rows:
    ...
    rank = int(peers.reset_index(drop=True).index[peers.reset_index(drop=True)["Company"] == selected][0] + 1) if selected in peers["Company"].values else None
    explanation(f"<b>What this shows:</b> The ranking compares historical risk-adjusted performance. <b>{selected}</b> ranks <b>#{rank} of {len(peers)}</b> by Sharpe ratio in the currently available pure-insurance peer set." if rank else f"<b>What this shows:</b> {selected} is not currently available in the pure-insurance peer subset, so a peer rank is not assigned.")
else:
