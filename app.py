...
render_sidebar_html(
    ...
    <div style="background: #1f2937; padding: 10px; border-radius: 8px; margin-top: 10px; color: #cbd5e1 !important; font-size: 0.85rem;">
        NSE Ticker<br><strong style="color: #ffffff !important;">{selected_ticker}</strong>
    </div>
    """
)
st.sidebar.markdown("---")
...
render_html(
    ...
    <div style="color:#64748b; font-size:0.9rem; margin-bottom:1rem;">
        Enter an investment amount and risk profile. InsureInvest ranks the available companies using historical return, risk-adjusted performance, downside risk and technical momentum.
    </div>
    """
)

rec_col1, rec_col2, rec_col3 = st.columns([1.2, 1, 1])
with rec_col1:
...
if "recommendation_failures" not in st.session_state:
    st.session_state.recommendation_failures = {}
if "recommendation_profile" not in st.session_state:
    st.session_state.recommendation_profile = "Moderate"

if analyze_button or st.session_state.recommendation_results is None or st.session_state.recommendation_profile != risk_profile:
    with st.spinner("Analyzing the investment universe..."):
        try:
            ...
            st.session_state.recommendation_failures = failures
            st.session_state.recommendation_profile = risk_profile
        except Exception as exc:
            st.error(f"The recommendation model could not complete the analysis: {exc}")
            st.session_state.recommendation_results = pd.DataFrame()

ranked_recommendations = st.session_state.recommendation_results

if isinstance(ranked_recommendations, pd.DataFrame) and not ranked_recommendations.empty:
    portfolio = build_portfolio(ranked_recommendations, investment_amount, max_companies=5)
    top_company = portfolio.index[0]
    top_score = float(portfolio.loc[top_company, "Investment Score"])

    render_html(
        f"""
        <div class="interpretation-card">
            <div class="interpretation-overall">Recommended portfolio — {risk_profile} risk</div>
            <div class="interpretation-overall-text">
                The model currently ranks <strong>{top_company}</strong> highest at <strong>{top_score:.1f}/100</strong>.
                The suggested allocation below distributes ₹{investment_amount:,.0f} across the top five successfully analysed companies.
            </div>
        </div>
        """
    )

    st.markdown("<div style='margin-top:1.3rem; font-size:1.05rem; font-weight:700; color:#111827;'>Suggested Allocation</div>", unsafe_allow_html=True)
    display_portfolio = portfolio[["Investment Score", "Allocation %", "Recommended Amount"]].copy()
    display_portfolio.insert(0, "Company", portfolio.index)
    display_portfolio["Investment Score"] = display_portfolio["Investment Score"].map(lambda x: f"{x:.1f}/100")
    display_portfolio["Allocation %"] = display_portfolio["Allocation %"].map(lambda x: f"{x:.1f}%")
    display_portfolio["Recommended Amount"] = display_portfolio["Recommended Amount"].map(lambda x: f"₹{x:,.0f}")
    display_portfolio.columns = ["Company", "Score", "Allocation", "Recommended Amount"]
    st.dataframe(display_portfolio, use_container_width=True, hide_index=True)

    st.markdown("<div style='margin-top:1.2rem; font-size:1.05rem; font-weight:700; color:#111827;'>Why these companies?</div>", unsafe_allow_html=True)
    insight_cols = st.columns(min(3, len(portfolio)))
    for i, (company, row) in enumerate(portfolio.iterrows()):
        with insight_cols[i % len(insight_cols)]:
            render_html(
                f"""
                <div class="interpretation-item" style="margin-bottom:1rem;">
                    <div class="interpretation-label">{company} — {row['Allocation %']:.1f}%</div>
                    <div class="interpretation-text">{recommendation_text(row)}</div>
                </div>
                """
            )

    with st.expander("How the recommendation score works"):
        st.markdown(
            """
            The model combines five dimensions: historical annual return, Sharpe ratio, annual volatility, maximum drawdown and technical momentum.

            Conservative investors place more emphasis on risk and downside protection. Moderate investors balance return and risk. Aggressive investors place more emphasis on return and momentum.

            Companies are ranked relative to the successfully loaded investment universe, and the selected amount is allocated across the top five according to their relative scores.
            """
        )

    failures = st.session_state.recommendation_failures
    if failures:
        st.caption(f"{len(failures)} companies could not be included because market data was unavailable. The recommendation uses only successfully loaded companies.")
else:
    st.warning("No companies could be analysed right now. Please retry after a short wait.")
...
render_html(
    ...
    <div class="company-meta">NSE: {selected_ticker} &nbsp; • &nbsp; Latest price: ₹{latest_price:,.2f} &nbsp; • &nbsp; Data through {latest_date.strftime('%d %B %Y')}</div>
    """
)
...
<div class="interpretation-card">
    ...
    <div class="interpretation-note">These signals are generated automatically from the selected company's historical market data. They are technical indicators, not a buy/sell recommendation.</div>
</div>
"""
...
with benchmark_col1:
    render_html(f"""<div class="metric-card"><div class="metric-label">{selected_company} Return</div><div class="metric-value">{stock_total_return:.2%}</div><div class="metric-description">Total historical return</div></div>""")
with benchmark_col2:
    render_html(f"""<div class="metric-card"><div class="metric-label">NIFTY 50 Return</div><div class="metric-value">{benchmark_total_return:.2%}</div><div class="metric-description">Total historical return</div></div>""")
with benchmark_col3:
    render_html(f"""<div class="metric-card"><div class="metric-label">Relative Performance</div><div class="metric-value">{relative_total_return:.2%}</div><div class="metric-description">Stock return minus NIFTY 50</div></div>""")
with benchmark_col4:
    render_html(f"""<div class="metric-card"><div class="metric-label">Benchmark Assessment</div><div class="metric-value" style="font-size:1.15rem;">{benchmark_assessment}</div><div class="metric-description">Based on annualized performance</div></div>""")
...
render_html('<div style="color:#64748b; font-size:0.9rem; margin-bottom:1rem;">Comparison of listed insurance companies using historical risk and return metrics over the same two-year period.</div>')
...
def load_peer_data():
    ...
    for company_name, ticker in INSURANCE_STOCKS.items():
        try:
            ...
            peer_results.append({"Company": company_name, "Ticker": ticker, "Annual Return": peer_metrics["Annual Return"], "Annual Volatility": peer_metrics["Annual Volatility"], "Sharpe Ratio": peer_metrics["Sharpe Ratio"], "Maximum Drawdown": peer_metrics["Maximum Drawdown"], "Beta": peer_metrics["Beta"]})
        except Exception:
            continue
...
if peer_df.empty:
    ...
    peer_fig = go.Figure()
    peer_fig.add_trace(go.Bar(x=other_peers["Peer Score"], y=other_peers["Company"], orientation="h", name="Other Insurance Companies", marker=dict(color="#CBD5E1"), text=[f"{value:.1f}" for value in other_peers["Peer Score"]], textposition="outside", hovertemplate="<b>%{y}</b><br>Peer Score: %{x:.1f}/100<extra></extra>"))
    ...
    if not selected_peer_row.empty:
        ...
        render_html(f"""<div class="interpretation-card"><div class="interpretation-overall">{selected_company}: Rank {selected_rank} of {total_companies}</div><div class="interpretation-overall-text">{peer_assessment} Its peer score is <strong>{selected_score:.1f}/100</strong>.</div><div class="interpretation-grid"><div class="interpretation-item"><div class="interpretation-label">Annual Return</div><div class="interpretation-text">{selected_peer_row["Annual Return"].iloc[0]:.2%}</div></div><div class="interpretation-item"><div class="interpretation-label">Annual Volatility</div><div class="interpretation-text">{selected_peer_row["Annual Volatility"].iloc[0]:.2%}</div></div><div class="interpretation-item"><div class="interpretation-label">Sharpe Ratio</div><div class="interpretation-text">{selected_peer_row["Sharpe Ratio"].iloc[0]:.2f}</div></div><div class="interpretation-item"><div class="interpretation-label">Maximum Drawdown</div><div class="interpretation-text">{selected_peer_row["Maximum Drawdown"].iloc[0]:.2%}</div></div><div class="interpretation-item"><div class="interpretation-label">Beta</div><div class="interpretation-text">{selected_peer_row["Beta"].iloc[0]:.2f}</div></div><div class="interpretation-item"><div class="interpretation-label">Peer Score</div><div class="interpretation-text">{selected_score:.1f}/100</div></div></div><div class="interpretation-note">The peer score is a relative historical ranking, not an investment recommendation. It combines return, risk-adjusted performance, volatility and maximum drawdown.</div></div>""")
    display_peer_df = peer_df[["Rank", "Company", "Annual Return", "Annual Volatility", "Sharpe Ratio", "Maximum Drawdown", "Beta", "Peer Score"]].copy()
    ...
    with st.expander("How is the Peer Score calculated?"):
        st.markdown("""
        The Peer Score is a relative score out of 100 based on the historical performance of the companies included in the prototype.
        ...
        **Scoring weights**
        ...
        Higher return and Sharpe Ratio receive higher scores. Lower volatility receives a higher score. A less severe maximum drawdown receives a higher score. Beta is displayed separately because higher or lower beta is not inherently better; it represents sensitivity to movements in the NIFTY 50.
        """)
...
render_html('<div style="color:#64748b; font-size:0.9rem; margin-bottom:1rem;">Portfolio optimization showing how diversification across insurance companies changes the risk-return trade-off.</div>')
...
if portfolio_returns.empty:
    ...
    if n_assets < 2:
        ...
        min_volatility_value = sharpe_array[min_volatility_index]
        ...
        frontier_fig.add_trace(go.Scatter(x=[max_sharpe_volatility * 100], y=[max_sharpe_return * 100], mode="markers", marker=dict(size=16, color="#16A34A", symbol="star"), name="Maximum Sharpe Portfolio", hovertemplate="<b>Maximum Sharpe Portfolio</b><br>Risk: %{x:.2f}%<br>Return: %{y:.2f}%<extra></extra>"))
        frontier_fig.add_trace(go.Scatter(x=[volatility_array[min_volatility_index] * 100], y=[min_volatility_return * 100], mode="markers", marker=dict(size=15, color="#2563EB", symbol="diamond"), name="Minimum Volatility Portfolio", hovertemplate="<b>Minimum Volatility Portfolio</b><br>Risk: %{x:.2f}%<br>Return: %{y:.2f}%<extra></extra>"))
        ...
        with col3:
            st.metric("Minimum Risk", f"{volatility_array[min_volatility_index]:.2%}")
        ...
        render_html(f"""<div class="interpretation-card"><div class="interpretation-overall">Portfolio Optimization Interpretation</div><div class="interpretation-overall-text">{portfolio_interpretation}</div><div class="interpretation-note">The efficient frontier is based on historical returns and covariance between the selected insurance companies. It does not guarantee future portfolio performance.</div></div>""")
        with st.expander("How to read the Efficient Frontier"):
            ...
            **Return** represents the historical annualized return implied by the portfolio weights.
            ...
            The key idea is diversification: combining different insurance stocks can produce a different risk-return profile from holding individual companies alone.
            """)
...
for i in range(DISPLAY_PATHS):
    simulation_fig.add_trace(go.Scatter(x=days, y=price_paths[:, i], mode="lines", line=dict(width=1), opacity=0.12, showlegend=False, hoverinfo="skip"))
...
with mc_col1:
    render_html(f"""<div class="metric-card"><div class="metric-label">Current Price</div><div class="metric-value">₹{current_price:,.2f}</div><div class="metric-description">Latest market price</div></div>""")
with mc_col2:
    render_html(f"""<div class="metric-card"><div class="metric-label">Median Price</div><div class="metric-value">₹{median_price:,.2f}</div><div class="metric-description">50th percentile after 1 year</div></div>""")
with mc_col3:
    render_html(f"""<div class="metric-card"><div class="metric-label">5th–95th Range</div><div class="metric-value" style="font-size:1.1rem;">₹{lower_price:,.0f} – ₹{upper_price:,.0f}</div><div class="metric-description">Simulated price range</div></div>""")
with mc_col4:
    render_html(f"""<div class="metric-card"><div class="metric-label">Probability Above Current</div><div class="metric-value">{probability_above_current:.1%}</div><div class="metric-description">Simulated paths ending higher</div></div>""")
...
positive_signals = sum([annual_return_value > 0, volatility_value <= 0.25, sharpe_value > 0.5, drawdown_value >= -0.25])
negative_signals = 4 - positive_signals
...
render_html(f"""<div class="interpretation-card"><div class="interpretation-overall">Overall Assessment</div><div class="interpretation-overall-text">{overall_text}</div><div class="interpretation-grid"><div class="interpretation-item"><div class="interpretation-label">Return</div><div class="interpretation-text">{return_text}</div></div><div class="interpretation-item"><div class="interpretation-label">Risk</div><div class="interpretation-text">{volatility_text}</div></div><div class="interpretation-item"><div class="interpretation-label">Risk-adjusted Performance</div><div class="interpretation-text">{sharpe_text}</div></div><div class="interpretation-item"><div class="interpretation-label">Downside Risk</div><div class="interpretation-text">{drawdown_text}</div></div><div class="interpretation-item"><div class="interpretation-label">Market Sensitivity</div><div class="interpretation-text">{beta_text}</div></div></div><div class="interpretation-note">Interpretation is generated automatically from the historical metrics displayed above. It is not a buy/sell recommendation.</div></div>""")
